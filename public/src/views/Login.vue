	...
	<div class="login-page">
		...
		<div class="login-wrap">
			...
			<div class="input-container">
				...
				<i class="close-icon icon" v-show="user.length>0" @click="user=''" ></i>
			</div>
			...
			<button class="login-btn" :class="{active:user.length>0&&pwd.length>0}" @click="login">登录</button>
...
<script>
	...
	export default {
		...
		methods:{
			login(){
				if (this.user=='aaaa'&&this.pwd=='aaaa') {
...
.login-page .title {
	...
	.login-wrap .login-btn {
		...
		box-shadow: 0 0.2rem 0.4rem 0 rgba(255, 62, 62, .2);
		text-align: center;
